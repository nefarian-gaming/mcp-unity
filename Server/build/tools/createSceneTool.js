...
export function createCreateSceneTool(server, mcpUnity, logger) {
    ...
    server.tool(toolNameForMcpServer, toolDescription, paramsSchema.shape, async (params) => {
        try {
 ...
 */
async function toolHandler(mcpUnity, params, unityMethod) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to execute scene creation in Unity (method: ${unityMethod}).`);
    }
    return {
        content: [{
